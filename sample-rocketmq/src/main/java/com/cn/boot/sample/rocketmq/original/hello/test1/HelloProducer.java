...
public class HelloProducer {
    ...
    public void init() throws MQClientException, RemotingException, InterruptedException, MQBrokerException {
        ...
        producer = new DefaultMQProducer("test1_producer_group");
        ...
        for (int i = 0; i < 5; i++) {
            byte[] body = ("Hello RocketMQ! test1_" + i).getBytes();
            Message message = new Message();
            message.setTopic("test1_topic"); // 主题
            message.setTags("test1"); // 标签
            message.setKeys("test1_" + i); // 消息唯一标识，用户自定义
