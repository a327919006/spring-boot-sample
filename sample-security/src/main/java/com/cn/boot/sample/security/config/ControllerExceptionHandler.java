...
public class ControllerExceptionHandler extends HttpStatusHandler {
    ...
    public Object resolveException(Exception e) {
        ...
        } else if (ExceptionUtil.isCausedBy(e, UnauthorizedException.class)) {
            log.error("【未认证异常】", e);
            UnauthorizedException exception = (UnauthorizedException) ExceptionUtil.getCausedBy(e, UnauthorizedException.class);
            return error(HttpStatus.UNAUTHORIZED, exception.getCode(), exception.getMsg());
        }
